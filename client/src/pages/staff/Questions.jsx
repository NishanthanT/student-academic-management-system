...
export default function Questions() {
  ...
  const handleDownloadPDF = () => {
    ...
    doc.text("UniExam", 105, 15, { align: "center" });

    doc.setFontSize(14);
    doc.setTextColor(0, 0, 0);
    doc.text(`Exam Paper: ${exam.title || "Untitled Exam"}`, 105, 25, {
      align: "center",
    });

    doc.setFontSize(11);
    doc.setTextColor(100, 110, 133);
    doc.text(
      `Module: ${exam.subject_code || "-"} - ${exam.subject_name || "-"}`,
      105,
      32,
      { align: "center" }
    );
    ...
    doc.text(`Total Marks: ${computed.expected || "-"}`, 105, 44, {
      align: "center",
    });
    ...
    rows.forEach((r, idx) => {
      ...
      const qText = `${r.question_no}. ${r.question_text}`;
      const marksStr = `(${r.marks} Marks)`;

      if (r.question_type === "MCQ") {
        const o = r.options || {};
        const optsStr = `   A) ${o.A || "-"}   B) ${o.B || "-"}   C) ${o.C || "-"}   D) ${o.D || "-"}`;
        body.push([qText, marksStr]);
        body.push([optsStr, ""]);
      } else {
        body.push([qText, marksStr]);
        body.push(["   Answer: ____________________", ""]);
      }
      body.push(["", ""]); // spacer
    });
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        0: { cellWidth: 160 },
        1: { cellWidth: 30, halign: "right", fontStyle: "italic" },
      },
      showHead: "never",
    });
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        `Page ${i} of ${pageCount} | Generated by UniExam System`,
        105,
        285,
        { align: "center" }
      );
    ...
    doc.save(`Exam_${exam.title || "Paper"}_${examId}.pdf`);
  };
    ...
    <div style={{ padding: 24, maxWidth: 1200, margin: "0 auto" }}>
      ...
      <div className="q-top">
        ...
        <div className="q-actions">
          ...
          {rows.some((r) => r.question_text?.trim() && r.db_id) && (
            <button
              className="q-btn"
