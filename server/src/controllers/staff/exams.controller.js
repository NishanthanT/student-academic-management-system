...
// ✅ GET /api/staff/exams
// Returns all exams for subjects this staff is assigned to.
// is_creator is true only if they created it.
exports.listMyExams = (req, res) => {
  ...
  const sql = `
    SELECT 
      ...
      CASE WHEN e.created_by_staff_id = ? THEN 1 ELSE 0 END AS is_creator,
    ...
    WHERE e.subject_id IN (
      SELECT subject_id FROM subject_staffs WHERE staff_id = ?
    )
    ORDER BY e.id DESC
  `;

  db.query(sql, [staffId, staffId], (err, rows) => {
    if (err) return res.status(500).json({ ok: false, message: "DB error" });
    res.json({ ok: true, data: rows });
  });
};
